...
*           Load default raylib font on initialization to be used by rlDrawText() and rlMeasureText().
*           If no default font loaded, rlDrawTextEx() and rlMeasureTextEx() are required.
*
...
*           On font atlas image generation [rlGenImageFontAtlas()], add a 3x3 pixels white rectangle
*           at the bottom-right corner of the atlas. It can be useful to for shapes drawing, to allow
*           drawing text and shapes with a single draw call [rlSetShapesTexture()].
*
...
#include <string.h>         // Required for: strcmp(), strstr(), strcpy(), strncpy() [Used in rlTextReplace()], sscanf() [Used in LoadBMFont()]
#include <stdarg.h>         // Required for: va_list, va_start(), vsprintf(), va_end() [Used in rlTextFormat()]
#include <ctype.h>          // Required for: toupper(), tolower() [Used in rlTextToUpper(), rlTextToLower()]
...
//----------------------------------------------------------------------------------
    ...
    #define RL_MAX_TEXT_BUFFER_LENGTH              1024        // rlSize of internal static buffers used on some functions:
                                                            // rlTextFormat(), rlTextSubtext(), rlTextToUpper(), rlTextToLower(), rlTextToPascal(), TextSplit()
#endif
...
// NOTE: Default font is loaded on rlInitWindow() and disposed on rlCloseWindow() [module: core]
...
extern void LoadFontDefault(void)
{
    ...
    rlTRACELOG(RL_LOG_INFO, "FONT: Default font loaded successfully (%i glyphs)", defaultFont.glyphCount);
}
...
rlFont rlLoadFont(const char *fileName)
{
...
#ifndef RL_FONT_TTF_DEFAULT_SIZE
    #define RL_FONT_TTF_DEFAULT_SIZE           32      // TTF font generation default char size (char-height)
#endif
#ifndef RL_FONT_TTF_DEFAULT_NUMCHARS
    #define RL_FONT_TTF_DEFAULT_NUMCHARS       95      // TTF font generation default charset: 95 glyphs (ASCII 32..126)
#endif
#ifndef RL_FONT_TTF_DEFAULT_FIRST_CHAR
    #define RL_FONT_TTF_DEFAULT_FIRST_CHAR     32      // TTF font generation default first char for image sprite font (32-Space)
#endif
#ifndef RL_FONT_TTF_DEFAULT_CHARS_PADDING
    #define RL_FONT_TTF_DEFAULT_CHARS_PADDING   4      // TTF font generation default chars padding
#endif

    rlFont font = { 0 };

#if defined(RL_SUPPORT_FILEFORMAT_TTF)
    if (rlIsFileExtension(fileName, ".ttf") || rlIsFileExtension(fileName, ".otf")) font = rlLoadFontEx(fileName, RL_FONT_TTF_DEFAULT_SIZE, NULL, RL_FONT_TTF_DEFAULT_NUMCHARS);
    else
#endif
#if defined(RL_SUPPORT_FILEFORMAT_FNT)
...
#endif
    {
        ...
        if (image.data != NULL) font = rlLoadFontFromImage(image, RL_MAGENTA, RL_FONT_TTF_DEFAULT_FIRST_CHAR);
        rlUnloadImage(image);
    }

    if (font.texture.id == 0)
    {
        rlTRACELOG(RL_LOG_WARNING, "FONT: [%s] Failed to load font texture -> Using default font", fileName);
        font = rlGetFontDefault();
    }
    else
    {
        rlSetTextureFilter(font.texture, RL_TEXTURE_FILTER_POINT);    // By default, we set point filter (the best performance)
        rlTRACELOG(RL_LOG_INFO, "FONT: Data loaded successfully (%i pixel size | %i glyphs)", RL_FONT_TTF_DEFAULT_SIZE, RL_FONT_TTF_DEFAULT_NUMCHARS);
    }
...
rlFont rlLoadFontEx(const char *fileName, int fontSize, int *codepoints, int codepointCount)
{
    rlFont font = { 0 };
...
rlFont rlLoadFontFromImage(rlImage image, rlColor key, int firstChar)
{
    ...
    rlRectangle tempCharRecs[RL_MAX_GLYPHS_FROM_IMAGE] = { 0 };
...
rlFont rlLoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount)
{
    rlFont font = { 0 };

    char fileExtLower[16] = { 0 };
    strcpy(fileExtLower, rlTextToLower(fileType));
        ...
        rlTextIsEqual(fileExtLower, ".otf"))
    {
        ...
        font.glyphPadding = 0;
        font.glyphs = rlLoadFontData(fileData, dataSize, font.baseSize, codepoints, font.glyphCount, RL_FONT_DEFAULT);

        if (font.glyphs != NULL)
        {
            font.glyphPadding = RL_FONT_TTF_DEFAULT_CHARS_PADDING;
            ...
            // Update glyphs[i].image to use alpha, required to be used on rlImageDrawText()
            for (int i = 0; i < font.glyphCount; i++)
            {
            ...
            rlTRACELOG(RL_LOG_INFO, "FONT: Data loaded successfully (%i pixel size | %i glyphs)", font.baseSize, font.glyphCount);
        }
        else font = rlGetFontDefault();
    }
#else
...
bool rlIsFontReady(rlFont font)
{
...
rlGlyphInfo *rlLoadFontData(const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount, int type)
{
    // NOTE: Using some SDF generation default values,
    // trades off precision with ability to handle *smaller* sizes
#ifndef RL_FONT_SDF_CHAR_PADDING
    #define RL_FONT_SDF_CHAR_PADDING            4      // SDF font generation char padding
#endif
#ifndef RL_FONT_SDF_ON_EDGE_VALUE
    #define RL_FONT_SDF_ON_EDGE_VALUE         128      // SDF font generation on edge value
#endif
#ifndef RL_FONT_SDF_PIXEL_DIST_SCALE
    #define RL_FONT_SDF_PIXEL_DIST_SCALE     64.0f     // SDF font generation pixel distance scale
#endif
#ifndef RL_FONT_BITMAP_ALPHA_THRESHOLD
    #define RL_FONT_BITMAP_ALPHA_THRESHOLD     80      // Bitmap (B&W) font generation alpha threshold
#endif
    ...
    if (fileData != NULL)
    {
        ...
        if (stbtt_InitFont(&fontInfo, (unsigned char *)fileData, 0))     // Initialize font for data reading
        {
            ...
            chars = (rlGlyphInfo *)RL_MALLOC(codepointCount*sizeof(rlGlyphInfo));
            ...
            for (int i = 0; i < codepointCount; i++)
            {
                ...
                else if (ch != 32) chars[i].image.data = stbtt_GetCodepointSDF(&fontInfo, scaleFactor, ch, RL_FONT_SDF_CHAR_PADDING, RL_FONT_SDF_ON_EDGE_VALUE, RL_FONT_SDF_PIXEL_DIST_SCALE, &chw, &chh, &chars[i].offsetX, &chars[i].offsetY);
                ...
                if (type == RL_FONT_BITMAP)
                {
                    ...
                    {
                        if (((unsigned char *)chars[i].image.data)[p] < RL_FONT_BITMAP_ALPHA_THRESHOLD) ((unsigned char *)chars[i].image.data)[p] = 0;
        ...
        else rlTRACELOG(RL_LOG_WARNING, "FONT: Failed to process TTF font data");
...
rlImage rlGenImageFontAtlas(const rlGlyphInfo *glyphs, rlRectangle **glyphRecs, int glyphCount, int fontSize, int padding, int packMethod)
{
    ...
    if (glyphs == NULL)
    {
        rlTRACELOG(RL_LOG_WARNING, "FONT: Provided chars info not valid, returning empty image atlas");
        return atlas;
    }
...
//#define RL_SUPPORT_FONT_ATLAS_SIZE_CONSERVATIVE
        ...
        for (int i = 0; i < glyphCount; i++)
        {
            ...
            if (offsetX >= (atlas.width - glyphs[i].image.width - 2*padding))
            {
                ...
                if (offsetY > (atlas.height - fontSize - padding))
                {
                    ...
                    {
                        rlTRACELOG(RL_LOG_WARNING, "FONT: Failed to package character (%i)", j);
    ...
    else if (packMethod == 1)  // Use Skyline rect packing algorithm (stb_pack_rect)
    {
        ...
        for (int i = 0; i < glyphCount; i++)
        {
            ...
            else rlTRACELOG(RL_LOG_WARNING, "FONT: Failed to package character (%i)", i);
        }
    ...
    // shapes and text can be backed into a single draw call: rlSetShapesTexture()
...
bool rlExportFontAsCode(rlFont font, const char *fileName)
{
    ...
    strcpy(fileNamePascal, rlTextToPascal(rlGetFileNameWithoutExt(fileName)));
    ...
    if (image.format != RL_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) rlTRACELOG(RL_LOG_WARNING, "rlFont export as code: rlFont image format is not RL_GRAY+ALPHA!");
...
#define RL_SUPPORT_COMPRESSED_FONT_ATLAS
    ...
    // WARNING: Data is compressed using raylib rlCompressData() DEFLATE,
    // it requires to be decompressed with raylib rlDecompressData(), that requires
...
#if defined(RL_SUPPORT_COMPRESSED_FONT_ATLAS)
    byteCount += sprintf(txtData + byteCount, "    // NOTE: Compressed font image data (DEFLATE), it requires rlDecompressData() function\n");
    byteCount += sprintf(txtData + byteCount, "    int fontDataSize_%s = 0;\n", fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "    unsigned char *data = rlDecompressData(fontData_%s, COMPRESSED_DATA_SIZE_FONT_%s, &fontDataSize_%s);\n", fileNamePascal, rlTextToUpper(fileNamePascal), fileNamePascal);
    ...
    byteCount += sprintf(txtData + byteCount, "    font.texture = rlLoadTextureFromImage(imFont);\n");
#if defined(RL_SUPPORT_COMPRESSED_FONT_ATLAS)
    byteCount += sprintf(txtData + byteCount, "    rlUnloadImage(imFont);  // Uncompressed data can be unloaded from memory\n\n");
#endif
...
//#define RL_SUPPORT_FONT_DATA_COPY
    ...
    if (success != 0) rlTRACELOG(RL_LOG_INFO, "FILEIO: [%s] rlFont as code exported successfully", fileName);
    else rlTRACELOG(RL_LOG_WARNING, "FILEIO: [%s] Failed to export font as code", fileName);
...
{
    rlColor color = RL_LIME;                         // Good FPS
    int fps = rlGetFPS();
...
void rlDrawTextEx(rlFont font, const char *text, rlVector2 position, float fontSize, float spacing, rlColor tint)
{
    ...
    for (int i = 0; i < size;)
    {
        ...
        if (codepoint == '\n')
        {
            // NOTE: Line spacing is a global variable, use rlSetTextLineSpacing() to setup
        ...
        else
        {
            ...
            {
                rlDrawTextCodepoint(font, codepoint, CAST(rlVector2){ position.x + textOffsetX, position.y + textOffsetY }, fontSize, tint);
            }
...
void rlDrawTextPro(rlFont font, const char *text, rlVector2 position, rlVector2 origin, float rotation, float fontSize, float spacing, rlColor tint)
{
    rlglPushMatrix();
        ...
        rlglTranslatef(position.x, position.y, 0.0f);
        rlglRotatef(rotation, 0.0f, 0.0f, 1.0f);
        rlglTranslatef(-origin.x, -origin.y, 0.0f);

        rlDrawTextEx(font, text, CAST(rlVector2){ 0.0f, 0.0f }, fontSize, spacing, tint);
...
void rlDrawTextCodepoint(rlFont font, int codepoint, rlVector2 position, float fontSize, rlColor tint)
{
    ...
    rlDrawTexturePro(font.texture, srcRec, dstRec, CAST(rlVector2){ 0, 0 }, 0.0f, tint);
}
...
void rlDrawTextCodepoints(rlFont font, const int *codepoints, int codepointCount, rlVector2 position, float fontSize, float spacing, rlColor tint)
{
    ...
    for (int i = 0; i < codepointCount; i++)
    {
        ...
        else
        {
            ...
            {
                rlDrawTextCodepoint(font, codepoints[i], CAST(rlVector2){ position.x + textOffsetX, position.y + textOffsetY }, fontSize, tint);
            }
...
rlVector2 rlMeasureTextEx(rlFont font, const char *text, float fontSize, float spacing)
{
    ...
    for (int i = 0; i < size;)
    {
        ...
        int next = 0;
        letter = rlGetCodepointNext(&text[i], &next);
...
char *rlTextReplace(char *text, const char *replace, const char *by)
{
...
static rlFont LoadBMFont(const char *fileName)
{
    ...
    if (lastSlash != NULL)
    {
        ...
        imPath = (char *)RL_CALLOC(rlTextLength(fileName) - rlTextLength(lastSlash) + rlTextLength(imFileName) + 4, 1);
        memcpy(imPath, fileName, rlTextLength(fileName) - rlTextLength(lastSlash) + 1);
        memcpy(imPath + rlTextLength(fileName) - rlTextLength(lastSlash) + 1, imFileName, rlTextLength(imFileName));
    }
    else imPath = imFileName;

    TRACELOGD("    > rlImage loading path: %s", imPath);

    rlImage imFont = rlLoadImage(imPath);
    ...
    for (int i = 0; i < glyphCount; i++)
    {
        ...
        {
            // Get character rectangle in the font atlas texture
            font.recs[i] = CAST(rlRectangle){ (float)charX, (float)charY, (float)charWidth, (float)charHeight };
            ...
            font.glyphs[i].image = rlImageFromImage(imFont, font.recs[i]);
        }
        else rlTRACELOG(RL_LOG_WARNING, "FONT: [%s] Some characters data not correctly provided", fileName);
    }
    ...
    if (font.texture.id == 0)
    {
        rlUnloadFont(font);
        font = rlGetFontDefault();
        rlTRACELOG(RL_LOG_WARNING, "FONT: [%s] Failed to load texture, reverted to default font", fileName);
    }
    else rlTRACELOG(RL_LOG_INFO, "FONT: [%s] rlFont loaded successfully (%i glyphs)", fileName, font.glyphCount);
