...
*           If not defined only some image editing functions supported: rlImageFormat(), rlImageAlphaMask(), rlImageResize*()
*
...
#include "utils.h"              // Required for: rlTRACELOG()
#include "rlgl.h"               // OpenGL abstraction layer to OpenGL 1.1, 3.3 or ES2

#include <stdlib.h>             // Required for: malloc(), free()
#include <string.h>             // Required for: strlen() [Used in rlImageTextEx()], strcmp() [Used in rlLoadImageFromMemory()]
#include <math.h>               // Required for: fabsf() [Used in rlDrawTextureRec()]
#include <stdio.h>              // Required for: sprintf() [Used in rlExportImageAsCode()]
...
#include "external/stb_image_resize2.h"  // Required for: stbir_resize_uint8_linear() [rlImageResize()]
...
// Defines and Macros
//----------------------------------------------------------------------------------
#ifndef RL_PIXELFORMAT_UNCOMPRESSED_R5G5B5A1_ALPHA_THRESHOLD
    #define RL_PIXELFORMAT_UNCOMPRESSED_R5G5B5A1_ALPHA_THRESHOLD  50    // Threshold over 255 to set alpha as 0
#endif
...
//----------------------------------------------------------------------------------
extern void LoadFontDefault(void);          // [Module: text] Loads default font, required by rlImageDrawText()
...
rlImage rlLoadImage(const char *fileName)
{
    ...
    if (fileData != NULL) image = rlLoadImageFromMemory(rlGetFileExtension(fileName), fileData, dataSize);
...
rlImage rlLoadImageSvg(const char *fileNameOrString, int width, int height)
{
    rlImage image = { 0 };
    ...
    if (fileNameOrString != NULL)
    {
        ...
        if (rlFileExists(fileNameOrString))
        {
            fileData = rlLoadFileData(fileNameOrString, &dataSize);
        ...
        if (isSvgStringValid && (fileData != fileNameOrString)) rlUnloadFileData(fileData);
    }
#else
    rlTRACELOG(RL_LOG_WARNING, "SVG image support not enabled, image can not be loaded");
#endif
...
rlImage rlLoadImageFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    else if ((strcmp(fileType, ".hdr") == 0) || (strcmp(fileType, ".HDR") == 0))
    {
        ...
        if (fileData != NULL)
        {
            ...
            {
                rlTRACELOG(RL_LOG_WARNING, "IMAGE: HDR file format not supported");
                rlUnloadImage(image);
            }
        }
#endif
    ...
    {
        image.data = rlgl_load_dds_from_memory(fileData, dataSize, &image.width, &image.height, &image.format, &image.mipmaps);
    }
#endif
#if defined(RL_SUPPORT_FILEFORMAT_PKM)
    else if ((strcmp(fileType, ".pkm") == 0) || (strcmp(fileType, ".PKM") == 0))
    {
        image.data = rlgl_load_pkm_from_memory(fileData, dataSize, &image.width, &image.height, &image.format, &image.mipmaps);
    }
#endif
#if defined(RL_SUPPORT_FILEFORMAT_KTX)
    else if ((strcmp(fileType, ".ktx") == 0) || (strcmp(fileType, ".KTX") == 0))
    {
        image.data = rlgl_load_ktx_from_memory(fileData, dataSize, &image.width, &image.height, &image.format, &image.mipmaps);
    }
#endif
#if defined(RL_SUPPORT_FILEFORMAT_PVR)
    else if ((strcmp(fileType, ".pvr") == 0) || (strcmp(fileType, ".PVR") == 0))
    {
        image.data = rlgl_load_pvr_from_memory(fileData, dataSize, &image.width, &image.height, &image.format, &image.mipmaps);
    }
#endif
#if defined(RL_SUPPORT_FILEFORMAT_ASTC)
    else if ((strcmp(fileType, ".astc") == 0) || (strcmp(fileType, ".ASTC") == 0))
    {
        image.data = rlgl_load_astc_from_memory(fileData, dataSize, &image.width, &image.height, &image.format, &image.mipmaps);
    }
#endif
    else rlTRACELOG(RL_LOG_WARNING, "IMAGE: Data format not supported");

    if (image.data != NULL) rlTRACELOG(RL_LOG_INFO, "IMAGE: Data loaded successfully (%ix%i | %s | %i mipmaps)", image.width, image.height, rlglGetPixelFormatName(image.format), image.mipmaps);
    else rlTRACELOG(RL_LOG_WARNING, "IMAGE: Failed to load image data");
...
rlImage rlLoadImageFromTexture(rlTexture2D texture)
{
    ...
    {
        image.data = rlglReadTexturePixels(texture.id, texture.width, texture.height, texture.format);
        ...
        if (image.data != NULL)
        {
            ...
            image.format = RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
#endif
            rlTRACELOG(RL_LOG_INFO, "TEXTURE: [ID %i] Pixel data retrieved successfully", texture.id);
        }
        else rlTRACELOG(RL_LOG_WARNING, "TEXTURE: [ID %i] Failed to retrieve pixel data", texture.id);
    }
    else rlTRACELOG(RL_LOG_WARNING, "TEXTURE: [ID %i] Failed to retrieve compressed pixel data", texture.id);
...
rlImage rlLoadImageFromScreen(void)
{
    ...
    image.width = rlGetScreenWidth();
    image.height = rlGetScreenHeight();
    image.mipmaps = 1;
    image.format = RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
    image.data = rlglReadScreenPixels(image.width, image.height);

    return image;
}

// Check if an image is ready
bool rlIsImageReady(rlImage image)
{
...
bool rlExportImage(rlImage image, const char *fileName)
{
    ...
    else if (rlIsFileExtension(fileName, ".qoi"))
    {
        ...
        else rlTRACELOG(RL_LOG_WARNING, "IMAGE: rlImage pixel format must be R8G8B8 or R8G8B8A8");
    ...
    {
        result = rlgl_save_ktx(fileName, image.data, image.width, image.height, image.format, image.mipmaps);
    }
#endif
    ...
    if (result != 0) rlTRACELOG(RL_LOG_INFO, "FILEIO: [%s] rlImage exported successfully", fileName);
    else rlTRACELOG(RL_LOG_WARNING, "FILEIO: [%s] Failed to export image", fileName);
...
bool rlExportImageAsCode(rlImage image, const char *fileName)
{
    ...
    if (success != 0) rlTRACELOG(RL_LOG_INFO, "FILEIO: [%s] rlImage as code exported successfully", fileName);
    else rlTRACELOG(RL_LOG_WARNING, "FILEIO: [%s] Failed to export image as code", fileName);
...
rlImage rlGenImageGradientLinear(int width, int height, int direction, rlColor start, rlColor end)
{
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            ...
            float factor = pos;
            factor = (factor > 1.0f)? 1.0f : factor;  // rlClamp to [0,1]
            factor = (factor < 0.0f)? 0.0f : factor;  // rlClamp to [0,1]
...
rlImage rlGenImageGradientSquare(int width, int height, float density, rlColor inner, rlColor outer)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            // rlNormalize the distances by the dimensions of the gradient rectangle
            ...
            // rlClamp the factor between 0 and 1
...
rlImage rlGenImageWhiteNoise(int width, int height, float factor)
{
    ...
    {
        if (rlGetRandomValue(0, 99) < (int)(factor*100.0f)) pixels[i] = RL_WHITE;
...
rlImage rlGenImagePerlinNoise(int width, int height, int offsetX, int offsetY, float scale)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            pixels[y*width + x] = CAST(rlColor){ (unsigned char)intensity, (unsigned char)intensity, (unsigned char)intensity, (unsigned char)255 };
        }
    }
...
rlImage rlGenImageCellular(int width, int height, int tileSize)
{
    ...
    for (int i = 0; i < seedCount; i++)
    {
        ...
        seeds[i] = CAST(rlVector2){ (float)x, (float)y };
    }
    ...
    for (int y = 0; y < height; y++)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            pixels[y*width + x] = CAST(rlColor){ (unsigned char)intensity, (unsigned char)intensity, (unsigned char)intensity, 255 };
        }
    }
...
rlImage rlImageCopy(rlImage image)
{
    ...
    {
        // NOTE: rlSize must be provided in bytes
...
void rlImageCrop(rlImage *image, rlRectangle crop)
{
    ...
    {
        rlTRACELOG(RL_LOG_WARNING, "IMAGE: Failed to crop, rectangle out of bounds");
        return;
    }

    if (image->mipmaps > 1) rlTRACELOG(RL_LOG_WARNING, "rlImage manipulation only applied to base mipmap level");
    if (image->format >= RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) rlTRACELOG(RL_LOG_WARNING, "rlImage manipulation not supported for compressed formats");
    else
    {
        int bytesPerPixel = rlGetPixelDataSize(1, 1, image->format);
...
void rlImageFormat(rlImage *image, int newFormat)
{
    ...
    if ((newFormat != 0) && (image->format != newFormat))
    {
            ...
            switch (image->format)
            {
                ...
                case RL_PIXELFORMAT_UNCOMPRESSED_R5G5B5A1:
                {
                    ...
                    for (int i = 0; i < image->width*image->height; i++)
                    {
                        ...
                        b = (unsigned char)(round(pixels[i].z*31.0f));
                        a = (pixels[i].w > ((float)RL_PIXELFORMAT_UNCOMPRESSED_R5G5B5A1_ALPHA_THRESHOLD/255.0f))? 1 : 0;
                ...
                {
                    // WARNING: rlImage is converted to GRAYSCALE equivalent 16bit
        ...
        else rlTRACELOG(RL_LOG_WARNING, "IMAGE: Data format is compressed, can not be converted");
    }
}
...
#else
    imText = rlGenImageColor(200, 60, RL_BLACK);     // Generating placeholder black image rectangle
    rlTRACELOG(RL_LOG_WARNING, "IMAGE: rlImageTextEx() requires module: rtext");
#endif
    return imText;
}
...
rlImage rlImageTextEx(rlFont font, const char *text, float fontSize, float spacing, rlColor tint)
{
    ...
    imText = rlGenImageColor((int)imSize.x, (int)imSize.y, RL_BLANK);
    ...
    for (int i = 0; i < size;)
    {
        ...
        else
        {
            if ((codepoint != ' ') && (codepoint != '\t'))
            {
                ...
                rlImageDraw(&imText, font.glyphs[index].image, CAST(rlRectangle){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
    ...
    if (textSize.y != imSize.y)
    {
        ...
        rlTRACELOG(RL_LOG_INFO, "IMAGE: Text scaled by factor: %f", scaleFactor);
...
void rlImageResize(rlImage *image, int newWidth, int newHeight)
{
        ...
        (image->format == RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8))
    {
        ...
        switch (image->format)
        {
            case RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: stbir_resize_uint8_linear((unsigned char *)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, (stbir_pixel_layout)1); break;
            case RL_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: stbir_resize_uint8_linear((unsigned char *)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, (stbir_pixel_layout)2); break;
            case RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8: stbir_resize_uint8_linear((unsigned char *)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, (stbir_pixel_layout)3); break;
            case RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8: stbir_resize_uint8_linear((unsigned char *)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, (stbir_pixel_layout)4); break;
            default: break;
        }
...
void rlImageAlphaMask(rlImage *image, rlImage alphaMask)
{
    ...
    {
        rlTRACELOG(RL_LOG_WARNING, "IMAGE: Alpha mask must be same size as image");
    }
    else if (image->format >= RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        rlTRACELOG(RL_LOG_WARNING, "IMAGE: Alpha mask can not be applied to compressed data formats");
    }
    else
    {
        // Force mask to be Grayscale
        rlImage mask = rlImageCopy(alphaMask);
        if (mask.format != RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) rlImageFormat(&mask, RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE);
        ...
        else
        {
            ...
            if (image->format != RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) rlImageFormat(image, RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8);
...
void rlImageMipmaps(rlImage *image)
{
    ...
    int mipSize = rlGetPixelDataSize(mipWidth, mipHeight, image->format);  // rlImage data size (in bytes)
    ...
    if (image->mipmaps < mipCount)
    {
        ...
        else rlTRACELOG(RL_LOG_WARNING, "IMAGE: Mipmaps required memory could not be allocated");

        // Pointer to allocated memory point where store next mipmap level data
        unsigned char *nextmip = (unsigned char *)image->data + rlGetPixelDataSize(image->width, image->height, image->format);
    ...
    else rlTRACELOG(RL_LOG_WARNING, "IMAGE: Mipmaps already available");
}
...
void rlImageDither(rlImage *image, int rBpp, int gBpp, int bBpp, int aBpp)
{
    ...
    {
        rlTRACELOG(RL_LOG_WARNING, "IMAGE: Compressed data formats can not be dithered");
        return;
    }

    if ((rBpp + gBpp + bBpp + aBpp) > 16)
    {
        rlTRACELOG(RL_LOG_WARNING, "IMAGE: Unsupported dithering bpps (%ibpp), only 16bpp or lower modes supported", (rBpp+gBpp+bBpp+aBpp));
    }
    else
    {
        rlColor *pixels = rlLoadImageColors(*image);
        ...
        {
            rlTRACELOG(RL_LOG_WARNING, "IMAGE: Format is already 16bpp or lower, dithering could have no effect");
        }
        ...
        {
            image->format = 0;
            rlTRACELOG(RL_LOG_WARNING, "IMAGE: Unsupported dithered OpenGL internal format: %ibpp (R%iG%iB%iA%i)", (rBpp+gBpp+bBpp+aBpp), rBpp, gBpp, bBpp, aBpp);
        }

        // NOTE: We will store the dithered data as unsigned short (16bpp)
        image->data = (unsigned short *)RL_MALLOC(image->width*image->height*sizeof(unsigned short));

        rlColor oldPixel = RL_WHITE;
        rlColor newPixel = RL_WHITE;
        ...
        for (int y = 0; y < image->height; y++)
        {
            for (int x = 0; x < image->width; x++)
            {
                ...
                // NOTE: New pixel obtained by bits truncate, it would be better to round values (check rlImageFormat())
...
{
    rlImageFormat(image, RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE);
}
...
// NOTE: Memory allocated should be freed using rlUnloadImageColors();
rlColor *rlLoadImageColors(rlImage image)
{
    ...
    if (image.format >= RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) rlTRACELOG(RL_LOG_WARNING, "IMAGE: Pixel data retrieval not supported for compressed image formats");
    else
    {
        if ((image.format == RL_PIXELFORMAT_UNCOMPRESSED_R32) ||
            (image.format == RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32) ||
            (image.format == RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32)) rlTRACELOG(RL_LOG_WARNING, "IMAGE: Pixel format converted from 32bit to 8bit per channel");

        if ((image.format == RL_PIXELFORMAT_UNCOMPRESSED_R16) ||
            (image.format == RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16) ||
            (image.format == RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16)) rlTRACELOG(RL_LOG_WARNING, "IMAGE: Pixel format converted from 16bit to 8bit per channel");
...
// NOTE: Memory allocated should be freed using rlUnloadImagePalette()
            ...
            if (pixels[i].a > 0)
            {
                ...
                if (!colorInPalette)
                {
                    ...
                    if (palCount >= maxPaletteSize)
                    {
                        ...
                        rlTRACELOG(RL_LOG_WARNING, "IMAGE: Palette is greater than %i colors", maxPaletteSize);
                    }
                }
            }
        }
...
// Unload color data loaded with rlLoadImageColors()
void rlUnloadImageColors(rlColor *colors)
{
    RL_FREE(colors);
}

// Unload colors palette loaded with rlLoadImagePalette()
...
rlRectangle rlGetImageAlphaBorder(rlImage image, float threshold)
{
    ...
    if (pixels != NULL)
    {
        ...
        {
            crop = CAST(rlRectangle){ (float)xMin, (float)yMin, (float)((xMax + 1) - xMin), (float)((yMax + 1) - yMin) };
        }
...
rlColor rlGetImageColor(rlImage image, int x, int y)
{
    ...
    if ((x >=0) && (x < image.width) && (y >= 0) && (y < image.height))
    {
        switch (image.format)
        {
            ...
            default: rlTRACELOG(RL_LOG_WARNING, "Compressed image format does not support color reading"); break;
        }
    }
    else rlTRACELOG(RL_LOG_WARNING, "Requested image pixel (%i, %i) out of bounds", x, y);
...
void rlImageDrawPixel(rlImage *dst, int x, int y, rlColor color)
{
    ...
    switch (dst->format)
    {
        ...
        case RL_PIXELFORMAT_UNCOMPRESSED_R5G5B5A1:
        {
            ...
            unsigned char b = (unsigned char)(round(coln.z*31.0f));
            unsigned char a = (coln.w > ((float)RL_PIXELFORMAT_UNCOMPRESSED_R5G5B5A1_ALPHA_THRESHOLD/255.0f))? 1 : 0;
...
void rlImageDrawLine(rlImage *dst, int startPosX, int startPosY, int endPosX, int endPosY, rlColor color)
{
    ...
    if (reversedXY)
    {
        ...
        rlImageDrawPixel(dst, startU, startV, color);     // At this point they are correctly ordered...
    }
    else
    {
        A = 2*absChangeInX;
        ...
        rlImageDrawPixel(dst, startV, startU, color);     // ... but need to be reversed here. Repeated in the main loop below
    }
    ...
    for (int u = startU + 1, v = startV; u <= endU; u++)
    {
        ...
        if (reversedXY) rlImageDrawPixel(dst, u, v, color);
        else rlImageDrawPixel(dst, v, u, color);
    }
}
...
{
    rlImageDrawLine(dst, (int)start.x, (int)start.y, (int)end.x, (int)end.y, color);
}
...
{
    rlImageDrawRectangleRec(dst, CAST(rlRectangle){ (float)posX, (float)posY, (float)width, (float)height }, color);
}
...
void rlImageDrawRectangleRec(rlImage *dst, rlRectangle rec, rlColor color)
{
    ...
    // rlClamp the size the the image bounds
...
void rlImageDraw(rlImage *dst, rlImage src, rlRectangle srcRec, rlRectangle dstRec, rlColor tint)
{
    ...
    if (dst->mipmaps > 1) rlTRACELOG(RL_LOG_WARNING, "rlImage drawing only applied to base mipmap level");
    if (dst->format >= RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) rlTRACELOG(RL_LOG_WARNING, "rlImage drawing not supported for compressed formats");
    else
    {
        ...
        if (((int)srcRec.width != (int)dstRec.width) || ((int)srcRec.height != (int)dstRec.height))
        {
            ...
            srcRec = CAST(rlRectangle){ 0, 0, (float)srcMod.width, (float)srcMod.height };
        ...
        //    [x] Optimize rlImageResize() for pixel format (alternative: rlImageResizeNN())
        //    [x] Optimize rlColorAlphaBlend() to avoid processing (alpha = 0) and (alpha = 1)
        //    [x] Optimize rlColorAlphaBlend() for faster operations (maybe avoiding divs?)
        //    [x] Consider fast path: no alpha blending required cases (src has no alpha)
        //    [x] Consider fast path: same src/dst format with no alpha -> direct line copy
        //    [-] rlGetPixelColor(): Get rlVector4 instead of rlColor, easier for rlColorAlphaBlend()
        //    [ ] Support f32bit channels drawing

        // TODO: Support RL_PIXELFORMAT_UNCOMPRESSED_R32, RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32, RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32 and 16-bit equivalents
...
#else
    rlTRACELOG(RL_LOG_WARNING, "IMAGE: rlImageDrawText() requires module: rtext");
#endif
}
...
void rlImageDrawTextEx(rlImage *dst, rlFont font, const char *text, rlVector2 position, float fontSize, float spacing, rlColor tint)
{
    ...
    rlImageDraw(dst, imText, srcRec, dstRec, RL_WHITE);
...
rlTexture2D rlLoadTextureFromImage(rlImage image)
{
    ...
    else rlTRACELOG(RL_LOG_WARNING, "IMAGE: Data is not valid to load texture");
...
rlTextureCubemap rlLoadTextureCubemap(rlImage image, int layout)
{
    ...
    if (layout == RL_CUBEMAP_LAYOUT_AUTO_DETECT)      // Try to automatically guess layout type
    {
        ...
        if (layout == RL_CUBEMAP_LAYOUT_LINE_VERTICAL) cubemap.width = image.height/6;
        if (layout == RL_CUBEMAP_LAYOUT_LINE_HORIZONTAL) cubemap.width = image.width/6;
        if (layout == RL_CUBEMAP_LAYOUT_CROSS_THREE_BY_FOUR) cubemap.width = image.width/3;
        if (layout == RL_CUBEMAP_LAYOUT_CROSS_FOUR_BY_THREE) cubemap.width = image.width/4;
        if (layout == RL_CUBEMAP_LAYOUT_PANORAMA) cubemap.width = image.width/4;
    }
    ...
    if (layout != RL_CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        for (int i = 0; i < 6; i++) faceRecs[i] = CAST(rlRectangle){ 0, 0, (float)size, (float)size };

        if (layout == RL_CUBEMAP_LAYOUT_LINE_VERTICAL)
        {
            faces = rlImageCopy(image);       // rlImage data already follows expected convention
        }
        ...
        else
        {
            ...
            faces = rlGenImageColor(size, size*6, RL_MAGENTA);
            rlImageFormat(&faces, image.format);

            // NOTE: rlImage formatting does not work with compressed textures

            for (int i = 0; i < 6; i++) rlImageDraw(&faces, image, faceRecs[i], CAST(rlRectangle){ 0, (float)size*i, (float)size, (float)size }, RL_WHITE);
        }

        // NOTE: Cubemap data is expected to be provided as 6 images in a single data array,
        // one after the other (that's a vertical image), following convention: +X, -X, +Y, -Y, +Z, -Z
        cubemap.id = rlglLoadTextureCubemap(faces.data, size, faces.format);
        if (cubemap.id == 0) rlTRACELOG(RL_LOG_WARNING, "IMAGE: Failed to load cubemap image");

        rlUnloadImage(faces);
    }
    else rlTRACELOG(RL_LOG_WARNING, "IMAGE: Failed to detect cubemap image layout");
...
rlRenderTexture2D rlLoadRenderTexture(int width, int height)
{
    ...
    target.id = rlglLoadFramebuffer(width, height);   // Load an empty framebuffer

    if (target.id > 0)
    {
        rlglEnableFramebuffer(target.id);

        // Create color texture (default to RGBA)
        target.texture.id = rlglLoadTexture(NULL, width, height, RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, 1);
        ...
        target.depth.id = rlglLoadTextureDepth(width, height, true);
        ...
        rlglFramebufferAttach(target.id, target.texture.id, RL_ATTACHMENT_COLOR_CHANNEL0, RL_ATTACHMENT_TEXTURE2D, 0);
        rlglFramebufferAttach(target.id, target.depth.id, RL_ATTACHMENT_DEPTH, RL_ATTACHMENT_RENDERBUFFER, 0);

        // Check if fbo is complete with attachments (valid)
        if (rlglFramebufferComplete(target.id)) rlTRACELOG(RL_LOG_INFO, "FBO: [ID %i] Framebuffer object created successfully", target.id);

        rlglDisableFramebuffer();
    }
    else rlTRACELOG(RL_LOG_WARNING, "FBO: Framebuffer object can not be created");

    return target;
}

// Check if a texture is ready
bool rlIsTextureReady(rlTexture2D texture)
{
...
void rlUnloadTexture(rlTexture2D texture)
{
    if (texture.id > 0)
    {
        rlglUnloadTexture(texture.id);

        rlTRACELOG(RL_LOG_INFO, "TEXTURE: [ID %i] Unloaded texture data from VRAM (GPU)", texture.id);
    }
}

// Check if a render texture is ready
bool rlIsRenderTextureReady(rlRenderTexture2D target)
{
    return ((target.id > 0) &&                  // Validate OpenGL id
            rlIsTextureReady(target.depth) &&     // Validate FBO depth texture/renderbuffer
            rlIsTextureReady(target.texture));    // Validate FBO texture
}
...
void rlUnloadRenderTexture(rlRenderTexture2D target)
{
    if (target.id > 0)
    {
        if (target.texture.id > 0)
        {
            ...
            rlglUnloadTexture(target.texture.id);
        }

        // NOTE: Depth texture/renderbuffer is automatically
        // queried and deleted before deleting framebuffer
        rlglUnloadFramebuffer(target.id);
    }
}
...
{
    rlglUpdateTexture(texture.id, 0, 0, texture.width, texture.height, texture.format, pixels);
}
...
{
    rlglUpdateTexture(texture.id, (int)rec.x, (int)rec.y, (int)rec.width, (int)rec.height, texture.format, pixels);
}
...
void rlGenTextureMipmaps(rlTexture2D *texture)
{
    ...
    rlglGenTextureMipmaps(texture->id, texture->width, texture->height, texture->format, &texture->mipmaps);
}
    ...
    switch (filter)
    {
        case RL_TEXTURE_FILTER_POINT:
        {
            if (texture.mipmaps > 1)
            {
                ...
                rlglTextureParameters(texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_MIP_NEAREST);

                // RL_TEXTURE_FILTER_NEAREST - tex filter: POINT (no filter), no mipmaps
                rlglTextureParameters(texture.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_NEAREST);
            }
            else
            {
                // RL_TEXTURE_FILTER_NEAREST - tex filter: POINT (no filter), no mipmaps
                rlglTextureParameters(texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_NEAREST);
                rlglTextureParameters(texture.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_NEAREST);
            }
        } break;
        case RL_TEXTURE_FILTER_BILINEAR:
        {
            if (texture.mipmaps > 1)
            {
                ...
                rlglTextureParameters(texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_LINEAR_MIP_NEAREST);

                // RL_TEXTURE_FILTER_LINEAR - tex filter: BILINEAR, no mipmaps
                rlglTextureParameters(texture.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_LINEAR);
            }
            else
            {
                // RL_TEXTURE_FILTER_LINEAR - tex filter: BILINEAR, no mipmaps
                rlglTextureParameters(texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_LINEAR);
                rlglTextureParameters(texture.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_LINEAR);
            }
        } break;
        case RL_TEXTURE_FILTER_TRILINEAR:
        {
            if (texture.mipmaps > 1)
            {
                ...
                rlglTextureParameters(texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_MIP_LINEAR);
            ...
            {
                rlTRACELOG(RL_LOG_WARNING, "TEXTURE: [ID %i] No mipmaps available for TRILINEAR texture filtering", texture.id);
        ...
        case RL_TEXTURE_FILTER_ANISOTROPIC_4X: rlglTextureParameters(texture.id, RL_TEXTURE_FILTER_ANISOTROPIC, 4); break;
        case RL_TEXTURE_FILTER_ANISOTROPIC_8X: rlglTextureParameters(texture.id, RL_TEXTURE_FILTER_ANISOTROPIC, 8); break;
        case RL_TEXTURE_FILTER_ANISOTROPIC_16X: rlglTextureParameters(texture.id, RL_TEXTURE_FILTER_ANISOTROPIC, 16); break;
        default: break;
    }
}
...
void rlSetTextureWrap(rlTexture2D texture, int wrap)
{
    switch (wrap)
    {
        case RL_TEXTURE_WRAP_REPEAT:
        {
            ...
            rlglTextureParameters(texture.id, RL_TEXTURE_WRAP_S, RL_TEXTURE_WRAP_REPEAT);
            rlglTextureParameters(texture.id, RL_TEXTURE_WRAP_T, RL_TEXTURE_WRAP_REPEAT);
        } break;
        case RL_TEXTURE_WRAP_CLAMP:
        {
            rlglTextureParameters(texture.id, RL_TEXTURE_WRAP_S, RL_TEXTURE_WRAP_CLAMP);
            rlglTextureParameters(texture.id, RL_TEXTURE_WRAP_T, RL_TEXTURE_WRAP_CLAMP);
        } break;
        case RL_TEXTURE_WRAP_MIRROR_REPEAT:
        {
            rlglTextureParameters(texture.id, RL_TEXTURE_WRAP_S, RL_TEXTURE_WRAP_MIRROR_REPEAT);
            rlglTextureParameters(texture.id, RL_TEXTURE_WRAP_T, RL_TEXTURE_WRAP_MIRROR_REPEAT);
        } break;
        case RL_TEXTURE_WRAP_MIRROR_CLAMP:
        {
            rlglTextureParameters(texture.id, RL_TEXTURE_WRAP_S, RL_TEXTURE_WRAP_MIRROR_CLAMP);
            rlglTextureParameters(texture.id, RL_TEXTURE_WRAP_T, RL_TEXTURE_WRAP_MIRROR_CLAMP);
        } break;
        default: break;
    }
}
...
{
    rlDrawTextureEx(texture, CAST(rlVector2){ (float)posX, (float)posY }, 0.0f, 1.0f, tint);
}
...
void rlDrawTexturePro(rlTexture2D texture, rlRectangle source, rlRectangle dest, rlVector2 origin, float rotation, rlColor tint)
{
    ...
    if (texture.id > 0)
    {
        ...
        if (rotation == 0.0f)
        {
            ...
            topRight = CAST(rlVector2){ x + dest.width, y };
            bottomLeft = CAST(rlVector2){ x, y + dest.height };
            bottomRight = CAST(rlVector2){ x + dest.width, y + dest.height };
        }
        else
        {
        ...
        rlglBegin(RL_QUADS);
            ...
            rlglNormal3f(0.0f, 0.0f, 1.0f);                          // Normal vector pointing towards viewer

            // Top-left corner for texture and quad
            if (flipX) rlglTexCoord2f((source.x + source.width)/width, source.y/height);
            else rlglTexCoord2f(source.x/width, source.y/height);
            rlglVertex2f(topLeft.x, topLeft.y);

            // Bottom-left corner for texture and quad
            if (flipX) rlglTexCoord2f((source.x + source.width)/width, (source.y + source.height)/height);
            else rlglTexCoord2f(source.x/width, (source.y + source.height)/height);
            rlglVertex2f(bottomLeft.x, bottomLeft.y);

            // Bottom-right corner for texture and quad
            if (flipX) rlglTexCoord2f(source.x/width, (source.y + source.height)/height);
            else rlglTexCoord2f((source.x + source.width)/width, (source.y + source.height)/height);
            rlglVertex2f(bottomRight.x, bottomRight.y);

            // Top-right corner for texture and quad
            if (flipX) rlglTexCoord2f(source.x/width, source.y/height);
            else rlglTexCoord2f((source.x + source.width)/width, source.y/height);
        ...
        rlglSetTexture(texture.id);
        rlglPushMatrix();
            rlglTranslatef(dest.x, dest.y, 0.0f);
            if (rotation != 0.0f) rlglRotatef(rotation, 0.0f, 0.0f, 1.0f);
            rlglTranslatef(-origin.x, -origin.y, 0.0f);
            ...
            rlglBegin(RL_QUADS);
                ...
                rlglVertex2f(0.0f, 0.0f);

                // Bottom-right corner for texture and quad
                if (flipX) rlglTexCoord2f((source.x + source.width)/width, (source.y + source.height)/height);
                else rlglTexCoord2f(source.x/width, (source.y + source.height)/height);
                rlglVertex2f(0.0f, dest.height);

                // Top-right corner for texture and quad
                if (flipX) rlglTexCoord2f(source.x/width, (source.y + source.height)/height);
                else rlglTexCoord2f((source.x + source.width)/width, (source.y + source.height)/height);
                rlglVertex2f(dest.width, dest.height);

                // Top-left corner for texture and quad
                if (flipX) rlglTexCoord2f(source.x/width, source.y/height);
                else rlglTexCoord2f((source.x + source.width)/width, source.y/height);
                rlglVertex2f(dest.width, 0.0f);
            rlglEnd();
        rlglPopMatrix();
        rlglSetTexture(0);
        */
...
void rlDrawTextureNPatch(rlTexture2D texture, rlNPatchInfo nPatchInfo, rlRectangle dest, rlVector2 origin, float rotation, rlColor tint)
{
    if (texture.id > 0)
    {
        ...
        rlglPushMatrix();
            rlglTranslatef(dest.x, dest.y, 0.0f);
            rlglRotatef(rotation, 0.0f, 0.0f, 1.0f);
            rlglTranslatef(-origin.x, -origin.y, 0.0f);
            ...
            rlglBegin(RL_QUADS);
                ...
                if (nPatchInfo.layout == RL_NPATCH_NINE_PATCH)
                {
                    // ------------------------------------------------------------
                    // TOP-LEFT QUAD
                    rlglTexCoord2f(coordA.x, coordB.y); rlglVertex2f(vertA.x, vertB.y);  // Bottom-left corner for texture and quad
                    rlglTexCoord2f(coordB.x, coordB.y); rlglVertex2f(vertB.x, vertB.y);  // Bottom-right corner for texture and quad
                    rlglTexCoord2f(coordB.x, coordA.y); rlglVertex2f(vertB.x, vertA.y);  // Top-right corner for texture and quad
                    rlglTexCoord2f(coordA.x, coordA.y); rlglVertex2f(vertA.x, vertA.y);  // Top-left corner for texture and quad
                    if (drawCenter)
                    {
                        // TOP-CENTER QUAD
                        rlglTexCoord2f(coordB.x, coordB.y); rlglVertex2f(vertB.x, vertB.y);  // Bottom-left corner for texture and quad
                        rlglTexCoord2f(coordC.x, coordB.y); rlglVertex2f(vertC.x, vertB.y);  // Bottom-right corner for texture and quad
                        rlglTexCoord2f(coordC.x, coordA.y); rlglVertex2f(vertC.x, vertA.y);  // Top-right corner for texture and quad
                        rlglTexCoord2f(coordB.x, coordA.y); rlglVertex2f(vertB.x, vertA.y);  // Top-left corner for texture and quad
                    }
                    // TOP-RIGHT QUAD
                    rlglTexCoord2f(coordC.x, coordB.y); rlglVertex2f(vertC.x, vertB.y);  // Bottom-left corner for texture and quad
                    rlglTexCoord2f(coordD.x, coordB.y); rlglVertex2f(vertD.x, vertB.y);  // Bottom-right corner for texture and quad
                    rlglTexCoord2f(coordD.x, coordA.y); rlglVertex2f(vertD.x, vertA.y);  // Top-right corner for texture and quad
                    rlglTexCoord2f(coordC.x, coordA.y); rlglVertex2f(vertC.x, vertA.y);  // Top-left corner for texture and quad
                    if (drawMiddle)
                    {
                        // ------------------------------------------------------------
                        // MIDDLE-LEFT QUAD
                        rlglTexCoord2f(coordA.x, coordC.y); rlglVertex2f(vertA.x, vertC.y);  // Bottom-left corner for texture and quad
                        rlglTexCoord2f(coordB.x, coordC.y); rlglVertex2f(vertB.x, vertC.y);  // Bottom-right corner for texture and quad
                        rlglTexCoord2f(coordB.x, coordB.y); rlglVertex2f(vertB.x, vertB.y);  // Top-right corner for texture and quad
                        rlglTexCoord2f(coordA.x, coordB.y); rlglVertex2f(vertA.x, vertB.y);  // Top-left corner for texture and quad
                        if (drawCenter)
                        {
                            // MIDDLE-CENTER QUAD
                            rlglTexCoord2f(coordB.x, coordC.y); rlglVertex2f(vertB.x, vertC.y);  // Bottom-left corner for texture and quad
                            rlglTexCoord2f(coordC.x, coordC.y); rlglVertex2f(vertC.x, vertC.y);  // Bottom-right corner for texture and quad
                            rlglTexCoord2f(coordC.x, coordB.y); rlglVertex2f(vertC.x, vertB.y);  // Top-right corner for texture and quad
                            rlglTexCoord2f(coordB.x, coordB.y); rlglVertex2f(vertB.x, vertB.y);  // Top-left corner for texture and quad
                        }

                        // MIDDLE-RIGHT QUAD
                        rlglTexCoord2f(coordC.x, coordC.y); rlglVertex2f(vertC.x, vertC.y);  // Bottom-left corner for texture and quad
                        rlglTexCoord2f(coordD.x, coordC.y); rlglVertex2f(vertD.x, vertC.y);  // Bottom-right corner for texture and quad
                        rlglTexCoord2f(coordD.x, coordB.y); rlglVertex2f(vertD.x, vertB.y);  // Top-right corner for texture and quad
                        rlglTexCoord2f(coordC.x, coordB.y); rlglVertex2f(vertC.x, vertB.y);  // Top-left corner for texture and quad
                    }

                    // ------------------------------------------------------------
                    // BOTTOM-LEFT QUAD
                    rlglTexCoord2f(coordA.x, coordD.y); rlglVertex2f(vertA.x, vertD.y);  // Bottom-left corner for texture and quad
                    rlglTexCoord2f(coordB.x, coordD.y); rlglVertex2f(vertB.x, vertD.y);  // Bottom-right corner for texture and quad
                    rlglTexCoord2f(coordB.x, coordC.y); rlglVertex2f(vertB.x, vertC.y);  // Top-right corner for texture and quad
                    rlglTexCoord2f(coordA.x, coordC.y); rlglVertex2f(vertA.x, vertC.y);  // Top-left corner for texture and quad
                    if (drawCenter)
                    {
                        // BOTTOM-CENTER QUAD
                        rlglTexCoord2f(coordB.x, coordD.y); rlglVertex2f(vertB.x, vertD.y);  // Bottom-left corner for texture and quad
                        rlglTexCoord2f(coordC.x, coordD.y); rlglVertex2f(vertC.x, vertD.y);  // Bottom-right corner for texture and quad
                        rlglTexCoord2f(coordC.x, coordC.y); rlglVertex2f(vertC.x, vertC.y);  // Top-right corner for texture and quad
                        rlglTexCoord2f(coordB.x, coordC.y); rlglVertex2f(vertB.x, vertC.y);  // Top-left corner for texture and quad
                    }

                    // BOTTOM-RIGHT QUAD
                    rlglTexCoord2f(coordC.x, coordD.y); rlglVertex2f(vertC.x, vertD.y);  // Bottom-left corner for texture and quad
                    rlglTexCoord2f(coordD.x, coordD.y); rlglVertex2f(vertD.x, vertD.y);  // Bottom-right corner for texture and quad
                    rlglTexCoord2f(coordD.x, coordC.y); rlglVertex2f(vertD.x, vertC.y);  // Top-right corner for texture and quad
                    rlglTexCoord2f(coordC.x, coordC.y); rlglVertex2f(vertC.x, vertC.y);  // Top-left corner for texture and quad
                }
...
rlColor rlFade(rlColor color, float alpha)
{
    ...
    return CAST(rlColor){ color.r, color.g, color.b, (unsigned char)(255.0f*alpha) };
}
...
#define COLORALPHABLEND_INTEGERS
    ...
    else
    {
        ...
        out = CAST(rlColor){ (unsigned char)(fout.x*255.0f), (unsigned char)(fout.y*255.0f), (unsigned char)(fout.z*255.0f), (unsigned char)(fout.w*255.0f) };
    }
#endif
...
rlColor rlGetPixelColor(void *srcPtr, int format)
{
    ...
    switch (format)
    {
        case RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: color = CAST(rlColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], 255 }; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: color = CAST(rlColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1] }; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R5G6B5:
        {
        ...
        case RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8: color = CAST(rlColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1], ((unsigned char *)srcPtr)[2], ((unsigned char *)srcPtr)[3] }; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8: color = CAST(rlColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1], ((unsigned char *)srcPtr)[2], 255 }; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R32:
        {
...
// NOTE: rlSize can be requested for rlImage or rlTexture data
int rlGetPixelDataSize(int width, int height, int format)
{
    int dataSize = 0;       // rlSize in bytes
