...
char* jar_xm_load_module(jar_xm_context_t* ctx, const char* moddata, size_t moddata_length, char* mempool) {
    ...
    for(uint16_t i = 0; i < ctx->module.num_instruments; ++i) {
        ...
        if(instr->num_samples > 0) {
            ...
            if(instr->vibrato_type == (jar_xm_waveform_type_t)2) {
                instr->vibrato_type = (jar_xm_waveform_type_t)1;
            } else if(instr->vibrato_type == (jar_xm_waveform_type_t)1) {
                instr->vibrato_type = (jar_xm_waveform_type_t)2;
            }
            instr->vibrato_sweep = (uint8_t)READ_U8(offset + 236);
            instr->vibrato_depth = (uint8_t)READ_U8(offset + 237);
            instr->vibrato_rate = (uint8_t)READ_U8(offset + 238);
            instr->volume_fadeout = (uint16_t)READ_U16(offset + 239);
