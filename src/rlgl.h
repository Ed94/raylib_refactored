...
*       initialized on glInit() to accumulate vertex data.
*
*       When an internal state change is required all the stored vertex data is renderer in batch,
*       additionally, DrawRenderBatchActive() could be called to force flushing of the batch.
*
...
*
*       Internal buffer (and resources) must be manually unloaded calling glClose().
*
*   CONFIGURATION:
...
*           required by any other module, use GetVersion() to check it
*
...
#ifndef RL_REFACTORED
#define RL_REFACTORED
#endif
...
typedef enum {
    ...
    BLEND_CUSTOM_SEPARATE            // Blend textures using custom src/dst factors (use SetBlendFactorsSeparate())
} BlendMode;
...
RLAPI void MatrixMode(int mode);                    // Choose the current matrix to be transformed
RLAPI void PushMatrix(void);                        // Push the current matrix to stack
RLAPI void PopMatrix(void);                         // Pop latest inserted matrix from stack
RLAPI void LoadIdentity(void);                      // Reset current matrix to identity matrix
...
RLAPI void MultMatrixf(const float *matf);                // Multiply the current matrix by another matrix
...
RLAPI void TexCoord2f(float x, float y);            // Define one vertex (texture coordinate) - 2 float
...
RLAPI bool EnableVertexArray(unsigned int vaoId);     // Enable vertex array (VAO, if supported)
RLAPI void DisableVertexArray(void);                  // Disable vertex array (VAO, if supported)
RLAPI void EnableVertexBuffer(unsigned int id);       // Enable vertex buffer (VBO)
RLAPI void DisableVertexBuffer(void);                 // Disable vertex buffer (VBO)
RLAPI void EnableVertexBufferElement(unsigned int id);// Enable vertex buffer element (VBO element)
RLAPI void DisableVertexBufferElement(void);          // Disable vertex buffer element (VBO element)
RLAPI void EnableVertexAttribute(unsigned int index); // Enable vertex attribute index
RLAPI void DisableVertexAttribute(unsigned int index);// Disable vertex attribute index
#if defined(GRAPHICS_API_OPENGL_11)
RLAPI void EnableStatePointer(int vertexAttribType, void *buffer);    // Enable attribute state pointer
RLAPI void DisableStatePointer(int vertexAttribType);                 // Disable attribute state pointer
#endif

// Textures state
RLAPI void ActiveTextureSlot(int slot);               // Select and active a texture slot
RLAPI void EnableTexture(unsigned int id);            // Enable texture
RLAPI void DisableTexture(void);                      // Disable texture
RLAPI void EnableTextureCubemap(unsigned int id);     // Enable texture cubemap
RLAPI void DisableTextureCubemap(void);               // Disable texture cubemap
RLAPI void TextureParameters(unsigned int id, int param, int value); // Set texture parameters (filter, wrap)
RLAPI void CubemapParameters(unsigned int id, int param, int value); // Set cubemap parameters (filter, wrap)

// Shader state
RLAPI void EnableShader(unsigned int id);             // Enable shader program
RLAPI void DisableShader(void);                       // Disable shader program

// Framebuffer state
RLAPI void EnableFramebuffer(unsigned int id);        // Enable render texture (fbo)
RLAPI void DisableFramebuffer(void);                  // Disable render texture (fbo), return to default framebuffer
RLAPI void ActiveDrawBuffers(int count);              // Activate multiple draw color buffers
RLAPI void BlitFramebuffer(int srcX, int srcY, int srcWidth, int srcHeight, int dstX, int dstY, int dstWidth, int dstHeight, int bufferMask); // Blit active framebuffer to main framebuffer

// General render state
RLAPI void EnableColorBlend(void);                     // Enable color blending
RLAPI void DisableColorBlend(void);                   // Disable color blending
RLAPI void EnableDepthTest(void);                     // Enable depth test
RLAPI void DisableDepthTest(void);                    // Disable depth test
RLAPI void EnableDepthMask(void);                     // Enable depth write
RLAPI void DisableDepthMask(void);                    // Disable depth write
RLAPI void EnableBackfaceCulling(void);               // Enable backface culling
RLAPI void DisableBackfaceCulling(void);              // Disable backface culling
RLAPI void SetCullFace(int mode);                     // Set face culling mode
RLAPI void EnableScissorTest(void);                   // Enable scissor test
RLAPI void DisableScissorTest(void);                  // Disable scissor test
RLAPI void Scissor(int x, int y, int width, int height); // Scissor test
RLAPI void EnableWireMode(void);                      // Enable wire mode
RLAPI void EnablePointMode(void);                     //  Enable point mode
RLAPI void DisableWireMode(void);                     // Disable wire mode ( and point ) maybe rename
RLAPI void SetLineWidth(float width);                 // Set the line drawing width
RLAPI float GetLineWidth(void);                       // Get the line drawing width
RLAPI void EnableSmoothLines(void);                   // Enable line aliasing
RLAPI void DisableSmoothLines(void);                  // Disable line aliasing
RLAPI void EnableStereoRender(void);                  // Enable stereo rendering
RLAPI void DisableStereoRender(void);                 // Disable stereo rendering
RLAPI bool IsStereoRenderEnabled(void);               // Check if stereo render is enabled

RLAPI void ClearColor(unsigned char r, unsigned char g, unsigned char b, unsigned char a); // Clear color buffer with color
RLAPI void ClearScreenBuffers(void);                  // Clear used screen buffers (color and depth)
RLAPI void CheckErrors(void);                         // Check and log OpenGL error codes
RLAPI void SetBlendMode(int mode);                    // Set blending mode
RLAPI void SetBlendFactors(int glSrcFactor, int glDstFactor, int glEquation); // Set blending mode factor and equation (using OpenGL factors)
RLAPI void SetBlendFactorsSeparate(int glSrcRGB, int glDstRGB, int glSrcAlpha, int glDstAlpha, int glEqRGB, int glEqAlpha); // Set blending mode factors and equations separately (using OpenGL factors)

//------------------------------------------------------------------------------------
// Functions Declaration - rlgl functionality
//------------------------------------------------------------------------------------
// rlgl initialization functions
RLAPI void glInit(int width, int height);             // Initialize rlgl (buffers, shaders, textures, states)
RLAPI void glClose(void);                             // De-initialize rlgl (buffers, shaders, textures)
RLAPI void LoadExtensions(void *loader);              // Load OpenGL extensions (loader function required)
RLAPI int GetVersion(void);                           // Get current OpenGL version
RLAPI void SetFramebufferWidth(int width);            // Set current framebuffer width
RLAPI int GetFramebufferWidth(void);                  // Get default framebuffer width
RLAPI void SetFramebufferHeight(int height);          // Set current framebuffer height
RLAPI int GetFramebufferHeight(void);                 // Get default framebuffer height

RLAPI unsigned int GetTextureIdDefault(void);         // Get default texture id
RLAPI unsigned int GetShaderIdDefault(void);          // Get default shader id
RLAPI int *GetShaderLocsDefault(void);                // Get default shader locations

// Render batch management
// NOTE: rlgl provides a default render batch to behave like OpenGL 1.1 immediate mode
// but this render batch API is exposed in case of custom batches are required
RLAPI RenderBatch LoadRenderBatch(int numBuffers, int bufferElements);  // Load a render batch system
RLAPI void UnloadRenderBatch(RenderBatch batch);                        // Unload render batch system
RLAPI void DrawRenderBatch(RenderBatch *batch);                         // Draw render batch data (Update->Draw->Reset)
RLAPI void SetRenderBatchActive(RenderBatch *batch);                    // Set the active render batch for rlgl (NULL for default internal)
RLAPI void DrawRenderBatchActive(void);                                   // Update and draw internal render batch
RLAPI bool CheckRenderBatchLimit(int vCount);                             // Check internal buffer overflow for a given number of vertex

RLAPI void SetTexture(unsigned int id);               // Set current texture for render batch and check buffers limits

//------------------------------------------------------------------------------------------------------------------------

// Vertex buffers management
RLAPI unsigned int LoadVertexArray(void);                               // Load vertex array (vao) if supported
RLAPI unsigned int LoadVertexBuffer(const void *buffer, int size, bool dynamic);            // Load a vertex buffer attribute
RLAPI unsigned int LoadVertexBufferElement(const void *buffer, int size, bool dynamic);     // Load a new attributes element buffer
RLAPI void UpdateVertexBuffer(unsigned int bufferId, const void *data, int dataSize, int offset);     // Update GPU buffer with new data
RLAPI void UpdateVertexBufferElements(unsigned int id, const void *data, int dataSize, int offset);   // Update vertex buffer elements with new data
RLAPI void UnloadVertexArray(unsigned int vaoId);
RLAPI void UnloadVertexBuffer(unsigned int vboId);
RLAPI void SetVertexAttribute(unsigned int index, int compSize, int type, bool normalized, int stride, const void *pointer);
RLAPI void SetVertexAttributeDivisor(unsigned int index, int divisor);
RLAPI void SetVertexAttributeDefault(int locIndex, const void *value, int attribType, int count); // Set vertex attribute default value
RLAPI void DrawVertexArray(int offset, int count);
RLAPI void DrawVertexArrayElements(int offset, int count, const void *buffer);
RLAPI void DrawVertexArrayInstanced(int offset, int count, int instances);
RLAPI void DrawVertexArrayElementsInstanced(int offset, int count, const void *buffer, int instances);

// Textures management
RLAPI unsigned int LoadTexture(const void *data, int width, int height, int format, int mipmapCount); // Load texture in GPU
RLAPI unsigned int LoadTextureDepth(int width, int height, bool useRenderBuffer);               // Load depth texture/renderbuffer (to be attached to fbo)
RLAPI unsigned int LoadTextureCubemap(const void *data, int size, int format);                        // Load texture cubemap
RLAPI void UpdateTexture(unsigned int id, int offsetX, int offsetY, int width, int height, int format, const void *data);  // Update GPU texture with new data
RLAPI void GetGlTextureFormats(int format, unsigned int *glInternalFormat, unsigned int *glFormat, unsigned int *glType);  // Get OpenGL internal formats
RLAPI const char *GetPixelFormatName(unsigned int format);              // Get name string for pixel format
RLAPI void UnloadTexture(unsigned int id);                              // Unload texture from GPU memory
RLAPI void GenTextureMipmaps(unsigned int id, int width, int height, int format, int *mipmaps); // Generate mipmap data for selected texture
RLAPI void *ReadTexturePixels(unsigned int id, int width, int height, int format);              // Read texture pixel data
RLAPI unsigned char *ReadScreenPixels(int width, int height);           // Read screen pixel data (color buffer)

// Framebuffer management (fbo)
RLAPI unsigned int LoadFramebuffer(int width, int height);              // Load an empty framebuffer
RLAPI void FramebufferAttach(unsigned int fboId, unsigned int texId, int attachType, int texType, int mipLevel);  // Attach texture/renderbuffer to a framebuffer
RLAPI bool FramebufferComplete(unsigned int id);                        // Verify framebuffer is complete
RLAPI void UnloadFramebuffer(unsigned int id);                          // Delete framebuffer from GPU

// Shaders management
RLAPI unsigned int LoadShaderCode(const char *vsCode, const char *fsCode);    // Load shader from code strings
RLAPI unsigned int CompileShader(const char *shaderCode, int type);           // Compile custom shader and return shader id (type: RL_VERTEX_SHADER, RL_FRAGMENT_SHADER, RL_COMPUTE_SHADER)
RLAPI unsigned int LoadShaderProgram(unsigned int vShaderId, unsigned int fShaderId); // Load custom shader program
RLAPI void UnloadShaderProgram(unsigned int id);                              // Unload shader program
RLAPI int GetLocationUniform(unsigned int shaderId, const char *uniformName); // Get shader location uniform
RLAPI int GetLocationAttrib(unsigned int shaderId, const char *attribName);   // Get shader location attribute
RLAPI void SetUniform(int locIndex, const void *value, int uniformType, int count);   // Set shader value uniform
RLAPI void SetUniformMatrix(int locIndex, Matrix mat);                        // Set shader value matrix
RLAPI void SetUniformSampler(int locIndex, unsigned int textureId);           // Set shader value sampler
RLAPI void SetShader(unsigned int id, int *locs);                             // Set shader currently active (id and locations)

// Compute shader management
RLAPI unsigned int LoadComputeShaderProgram(unsigned int shaderId);           // Load compute shader program
RLAPI void ComputeShaderDispatch(unsigned int groupX, unsigned int groupY, unsigned int groupZ);  // Dispatch compute shader (equivalent to *draw* for graphics pipeline)

// Shader buffer storage object management (ssbo)
RLAPI unsigned int LoadShaderBuffer(unsigned int size, const void *data, int usageHint); // Load shader storage buffer object (SSBO)
RLAPI void UnloadShaderBuffer(unsigned int ssboId);                           // Unload shader storage buffer object (SSBO)
RLAPI void UpdateShaderBuffer(unsigned int id, const void *data, unsigned int dataSize, unsigned int offset); // Update SSBO buffer data
RLAPI void BindShaderBuffer(unsigned int id, unsigned int index);             // Bind SSBO buffer
RLAPI void ReadShaderBuffer(unsigned int id, void *dest, unsigned int count, unsigned int offset); // Read SSBO buffer data (GPU->CPU)
RLAPI void CopyShaderBuffer(unsigned int destId, unsigned int srcId, unsigned int destOffset, unsigned int srcOffset, unsigned int count); // Copy SSBO data between buffers
RLAPI unsigned int GetShaderBufferSize(unsigned int id);                      // Get SSBO buffer size

// Buffer management
RLAPI void BindImageTexture(unsigned int id, unsigned int index, int format, bool readonly);  // Bind image texture

// Matrix state management
RLAPI Matrix GetMatrixModelview(void);                                  // Get internal modelview matrix
RLAPI Matrix GetMatrixProjection(void);                                 // Get internal projection matrix
RLAPI Matrix GetMatrixTransform(void);                                  // Get internal accumulated transform matrix
RLAPI Matrix GetMatrixProjectionStereo(int eye);                        // Get internal projection matrix for stereo render (selected eye)
RLAPI Matrix GetMatrixViewOffsetStereo(int eye);                        // Get internal view offset matrix for stereo render (selected eye)
RLAPI void SetMatrixProjection(Matrix proj);                            // Set a custom projection matrix (replaces internal projection matrix)
RLAPI void SetMatrixModelview(Matrix view);                             // Set a custom modelview matrix (replaces internal modelview matrix)
RLAPI void SetMatrixProjectionStereo(Matrix right, Matrix left);        // Set eyes projection matrices for stereo rendering
RLAPI void SetMatrixViewOffsetStereo(Matrix right, Matrix left);        // Set eyes view offsets matrices for stereo rendering

// Quick and dirty cube/quad buffers load->draw->unload
RLAPI void LoadDrawCube(void);     // Load and draw a cube
RLAPI void LoadDrawQuad(void);     // Load and draw a quad
...
#include <string.h>                     // Required for: strcmp(), strlen() [Used in glInit(), on extensions loading]
...
//----------------------------------------------------------------------------------
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
static void LoadShaderDefault(void);      // Load default shader
...
static Matrix MatrixIdentity(void);                       // Get identity matrix
static Matrix MatrixMultiply(Matrix left, Matrix right);  // Multiply two matrices
...
void MultMatrixf(const float *matf) { glMultMatrixf(matf); }
#endif
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
{
    *GLOBAL_DATA.State.currentMatrix = MatrixIdentity();
}
...
void Translatef(float x, float y, float z)
{
    ...
    *GLOBAL_DATA.State.currentMatrix = MatrixMultiply(matTranslation, *GLOBAL_DATA.State.currentMatrix);
}
...
void Rotatef(float angle, float x, float y, float z)
{
    ...
    // NOTE: We transpose matrix with multiplication order
    *GLOBAL_DATA.State.currentMatrix = MatrixMultiply(matRotation, *GLOBAL_DATA.State.currentMatrix);
}
...
void Scalef(float x, float y, float z)
{
    ...
    // NOTE: We transpose matrix with multiplication order
    *GLOBAL_DATA.State.currentMatrix = MatrixMultiply(matScale, *GLOBAL_DATA.State.currentMatrix);
}
...
void MultMatrixf(const float *matf)
{
    ...
    *GLOBAL_DATA.State.currentMatrix = MatrixMultiply(*GLOBAL_DATA.State.currentMatrix, mat);
}
...
void Frustum(double left, double right, double bottom, double top, double znear, double zfar)
{
    ...
    *GLOBAL_DATA.State.currentMatrix = MatrixMultiply(*GLOBAL_DATA.State.currentMatrix, matFrustum);
}
...
void Ortho(double left, double right, double bottom, double top, double znear, double zfar)
{
    ...
    *GLOBAL_DATA.State.currentMatrix = MatrixMultiply(*GLOBAL_DATA.State.currentMatrix, matOrtho);
}
#endif
...
void TexCoord2f(float x, float y) { glTexCoord2f(x, y); }
...
void Begin(int mode)
{
    ...
    if (GLOBAL_DATA.currentBatch->draws[GLOBAL_DATA.currentBatch->drawCounter - 1].mode != mode)
    {
        if (GLOBAL_DATA.currentBatch->draws[GLOBAL_DATA.currentBatch->drawCounter - 1].vertexCount > 0)
        {
            ...
            if (!CheckRenderBatchLimit(GLOBAL_DATA.currentBatch->draws[GLOBAL_DATA.currentBatch->drawCounter - 1].vertexAlignment))
            {
        ...
        if (GLOBAL_DATA.currentBatch->drawCounter >= RL_DEFAULT_BATCH_DRAWCALLS) DrawRenderBatch(GLOBAL_DATA.currentBatch);
...
void SetTexture(unsigned int id)
{
    if (id == 0)
    {
        ...
        {
            DrawRenderBatch(GLOBAL_DATA.currentBatch);
        }
#endif
    }
    else
...
void SetBlendMode(int mode)
{
    ...
    if ((GLOBAL_DATA.State.currentBlendMode != mode) || ((mode == BLEND_CUSTOM || mode == BLEND_CUSTOM_SEPARATE) && GLOBAL_DATA.State.glCustomBlendModeModified))
    {
        ...
        switch (mode)
        {
            ...
            case BLEND_CUSTOM:
            {
                // NOTE: Using GL blend src/dst factors and GL equation configured with SetBlendFactors()
            ...
            {
                // NOTE: Using GL blend src/dst factors and GL equation configured with SetBlendFactorsSeparate()
...
void glInit(int width, int height)
{
    ...
    GLOBAL_DATA.State.defaultTextureId = LoadTexture(pixels, 1, 1, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, 1);
    ...
    GLOBAL_DATA.defaultBatch = LoadRenderBatch(RL_DEFAULT_BATCH_BUFFERS, RL_DEFAULT_BATCH_BUFFER_ELEMENTS);
    GLOBAL_DATA.currentBatch = &GLOBAL_DATA.defaultBatch;

    // Init stack matrices (emulating OpenGL 1.1)
    for (int i = 0; i < RL_MAX_MATRIX_STACK_SIZE; i++) GLOBAL_DATA.State.stack[i] = MatrixIdentity();

    // Init internal matrices
    GLOBAL_DATA.State.transform = MatrixIdentity();
    GLOBAL_DATA.State.projection = MatrixIdentity();
    GLOBAL_DATA.State.modelview = MatrixIdentity();
    GLOBAL_DATA.State.currentMatrix = &GLOBAL_DATA.State.modelview;
#endif  // GRAPHICS_API_OPENGL_33 || GRAPHICS_API_OPENGL_ES2
...
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    UnloadRenderBatch(GLOBAL_DATA.defaultBatch);
...
RenderBatch LoadRenderBatch(int numBuffers, int bufferElements)
{
    ...
    //--------------------------------------------------------------------------------------------
    batch.draws = (DrawCall *)RL_MALLOC(RL_DEFAULT_BATCH_DRAWCALLS*sizeof(DrawCall));
    ...
    for (int i = 0; i < RL_DEFAULT_BATCH_DRAWCALLS; i++)
    {
        ...
        //batch.draws[i].GLOBAL_DATA.State.projection = MatrixIdentity();
        //batch.draws[i].GLOBAL_DATA.State.modelview = MatrixIdentity();
    }
...
void DrawRenderBatch(RenderBatch *batch)
{
    ...
    for (int eye = 0; eye < eyeCount; eye++)
    {
        if (eyeCount == 2)
        {
            ...
            SetMatrixModelview(MatrixMultiply(matModelView, GLOBAL_DATA.State.viewOffsetStereo[eye]));
            // Set current eye projection matrix
            SetMatrixProjection(GLOBAL_DATA.State.projectionStereo[eye]);
        }
        ...
        if (GLOBAL_DATA.State.vertexCounter > 0)
        {
            ...
            Matrix matMVP = MatrixMultiply(GLOBAL_DATA.State.modelview, GLOBAL_DATA.State.projection);
...
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    DrawRenderBatch(GLOBAL_DATA.currentBatch);    // NOTE: Stereo rendering is checked inside
#endif
}
...
unsigned int LoadTexture(const void *data, int width, int height, int format, int mipmapCount)
{
    ...
    if (id > 0) RL_TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Texture loaded successfully (%ix%i | %s | %i mipmaps)", id, width, height, GetPixelFormatName(format), mipmapCount);
...
unsigned int LoadTextureCubemap(const void *data, int size, int format)
{
    unsigned int id = 0;
...
void *ReadTexturePixels(unsigned int id, int width, int height, int format)
{
    ...
    unsigned int fboId = LoadFramebuffer(width, height);
    ...
    pixels = (unsigned char *)RL_MALLOC(GetPixelDataSize(width, height, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8));
...
unsigned int LoadFramebuffer(int width, int height)
{
...
void SetVertexAttribute(unsigned int index, int compSize, int type, bool normalized, int stride, const void *pointer)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
void UnloadShaderBuffer(unsigned int ssboId)
{
#if defined(GRAPHICS_API_OPENGL_43)
...
void UpdateShaderBuffer(unsigned int id, const void *data, unsigned int dataSize, unsigned int offset)
{
#if defined(GRAPHICS_API_OPENGL_43)
...
unsigned int GetShaderBufferSize(unsigned int id)
{
    long long size = 0;
...
void ReadShaderBuffer(unsigned int id, void *dest, unsigned int count, unsigned int offset)
{
#if defined(GRAPHICS_API_OPENGL_43)
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, id);
...
// Bind SSBO buffer
void BindShaderBuffer(unsigned int id, unsigned int index)
{
#if defined(GRAPHICS_API_OPENGL_43)
...
void CopyShaderBuffer(unsigned int destId, unsigned int srcId, unsigned int destOffset, unsigned int srcOffset, unsigned int count)
{
#if defined(GRAPHICS_API_OPENGL_43)
...
Matrix GetMatrixTransform(void)
{
    ...
    //for (int i = GLOBAL_DATA.State.stackCounter; i > 0; i--) matStackTransform = MatrixMultiply(GLOBAL_DATA.State.stack[i], matStackTransform);
...
RLAPI Matrix GetMatrixProjectionStereo(int eye)
{
    Matrix mat = MatrixIdentity();
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
RLAPI Matrix GetMatrixViewOffsetStereo(int eye)
{
    Matrix mat = MatrixIdentity();
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
static void LoadShaderDefault(void)
{
    ...
    GLOBAL_DATA.State.defaultVShaderId = CompileShader(defaultVShaderCode, GL_VERTEX_SHADER);     // Compile default vertex shader
    GLOBAL_DATA.State.defaultFShaderId = CompileShader(defaultFShaderCode, GL_FRAGMENT_SHADER);   // Compile default fragment shader

    GLOBAL_DATA.State.defaultShaderId = LoadShaderProgram(GLOBAL_DATA.State.defaultVShaderId, GLOBAL_DATA.State.defaultFShaderId);
